...
def _process_data_from_proxy(
    stops_dict, stoptimes_dict, footpath_dict,
    routes_by_stop_dict, idx_by_route_stop_dict, stop_ids_set
):
    """Преобразует прокси-объекты в обычные словари и множества."""
    return (
        dict(stops_dict.items()),
        dict(stoptimes_dict.items()),
        dict(footpath_dict.items()),
        dict(routes_by_stop_dict.items()),
        dict(idx_by_route_stop_dict.items()),
        set(stop_ids_set._getvalue())
    )

def create_output_in_process(mode, time_str, protocol_type, timetable_mode, sources, params,
                             layer_buildings_path,
                             stops_dict_proxy, stoptimes_dict_proxy, footpath_dict_proxy,
                             routes_by_stop_dict_proxy, idx_by_route_stop_dict_proxy,
                             stop_ids_set_proxy, path_to_pkl, process_id):
    ...
    # Преобразуем прокси в обычные объекты
    dictionary = _process_data_from_proxy(
        stops_dict_proxy, stoptimes_dict_proxy, footpath_dict_proxy,
        routes_by_stop_dict_proxy, idx_by_route_stop_dict_proxy,
        stop_ids_set_proxy
    )
...
class raptor_multiprocessing:
    ...
    def run_parallel(self, gen_instance, mode, time_start_str, protocol_type, timetable_mode, all_sources, num_processes=None):
        ...
        with MyManager() as manager:
            # --- Замер времени на создание прокси-объектов ---
            proxy_creation_start = time.time()
            stops_dict_proxy = manager.dict(stops_dict)
            stoptimes_dict_proxy = manager.dict(stoptimes_dict)
            footpath_dict_proxy = manager.dict(footpath_dict)
            routes_by_stop_dict_proxy = manager.dict(routes_by_stop_dict)
            idx_by_route_stop_dict_proxy = manager.dict(idx_by_route_stop_dict)
            stop_ids_set_proxy = manager.set(stop_ids_set)
            proxy_creation_end = time.time()
            print(f"Прокси-объекты созданы. Время создания: {proxy_creation_end - proxy_creation_start:.2f} сек.")
            ...
            for i, chunk in enumerate(source_chunks):
                p = Process(
                    ...
                    args=(
                        ...
                        gen_instance.layer_buildings_path, stops_dict_proxy, stoptimes_dict_proxy, footpath_dict_proxy,
                        routes_by_stop_dict_proxy, idx_by_route_stop_dict_proxy, stop_ids_set_proxy,
